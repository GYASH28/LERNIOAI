  ...
  for (const palette of palettes) {
    ...
    for (const appearance of appearances) {
      const snapshot = await page.evaluate(
        ({ nextPalette, nextAppearance, requiredTokens }) => {
      ...
      for (const token of REQUIRED_TOKENS) {
        expect(snapshot.tokens[token], `${palette}/${appearance} must resolve ${token}`).not.toBe('')
      }
      expect(snapshot.colorScheme).toContain(appearance)
      expect(
        snapshot.scrollWidth,
        `${palette}/${appearance} introduces horizontal overflow (${snapshot.scrollWidth}px > ${snapshot.clientWidth}px)`,
