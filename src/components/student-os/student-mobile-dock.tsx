...
const primaryItems: Array<{ href?: string; label: string; icon: LucideIcon; action?: 'more' }> = [
  { href: '/student-os', label: 'Home', icon: Home },
...
const moreItems = [
  { href: '/revision', label: 'Revision', description: 'Due flashcards and recall', icon: RotateCcw },
    ...
    <>
        ...
        <div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-sm md:hidden" role="dialog" aria-modal="true" aria-label="More learning tools">
          ...
          <div className="absolute inset-x-0 bottom-0 max-h-[78dvh] overflow-y-auto rounded-t-3xl border border-border bg-background p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl">
            ...
            <div className="flex items-start justify-between gap-4 px-1">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-primary">Student tools</p>
                <h2 className="mt-1 text-xl font-bold">More ways to learn</h2>
              </div>
              <button type="button" onClick={() => setMoreOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-xl border border-border hover:bg-accent" aria-label="Close more menu">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {moreItems.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className={cn('flex items-start gap-3 rounded-2xl border p-4', active ? 'border-primary bg-primary/10' : 'border-border bg-card hover:bg-accent/60')}>
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><item.icon className="h-5 w-5" /></span>
                    <span>
      ...
      <nav className="fixed inset-x-2 bottom-[max(0.5rem,env(safe-area-inset-bottom))] z-40 grid grid-cols-5 rounded-2xl border border-border bg-background/95 p-1.5 shadow-2xl backdrop-blur md:hidden" aria-label="Student mobile navigation">
        {primaryItems.map((item) => {
          const active = item.href ? pathname === item.href || pathname.startsWith(`${item.href}/`) : moreOpen
          ...
          if (item.action === 'more') {
            return (
              <button key={item.label} type="button" onClick={() => setMoreOpen(true)} className={sharedClass} aria-expanded={moreOpen}>
          ...
          return (
            <Link key={item.href} href={item.href || '/student-os'} className={sharedClass}>
              <item.icon className="h-5 w-5" />
              <span>{item.label}</span>
            </Link>
          )
