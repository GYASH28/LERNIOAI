    ...
    return (
      <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-6 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          You scored <span className="font-bold text-foreground">{score}</span> / {questions.length} ({pct}%)
        </p>
          ...
          {questions.map((q, i) => (
            <span
              ...
              className={`h-2 w-2 rounded-full ${
                answers[i] === q.answer ? 'bg-green-500' : 'bg-red-500'
              }`}
    ...
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          Q{currentQ + 1} / {questions.length} · Score: {answers.filter((a, i) => a === questions[i].answer).length}
        </span>
      </div>
              ...
              className={`flex w-full items-center gap-2 rounded-md border px-3 py-2 text-left text-sm transition-colors ${
                showCorrect
                  ...
                  : showWrong
                    ...
                    : 'border-border hover:bg-accent/50 disabled:cursor-default'
              }`}
              type="button"
            >
      ...
      {showResult && (
        <div className="mt-3 rounded-md bg-muted/50 p-3">
          ...
          <button
            onClick={handleNext}
            className="mt-2 inline-flex items-center gap-1 rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90 transition-colors"
            type="button"
          >
