...
import { RefreshCw, Search, UserCheck, UserCog, UserX, Users, Trash2, MailPlus, AlertTriangle } from 'lucide-react'
...
export function SimplePeopleRoles() {
  ...
  async function deleteUser(user: UserRow) {
    if (!window.confirm(`Delete ${user.name} (${user.email})? This permanently removes the account and all related data.`)) return
    setBusy(true)
    setMessage('')
    try {
      ...
      setMessage(error instanceof Error ? error.message : 'Could not delete this account.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div className="flex justify-center">
      <Link href="/admin/invitations" className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-6 py-3 text-sm font-semibold hover:border-primary/40 hover:bg-accent/5">
