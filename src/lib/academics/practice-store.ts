...
export async function getPracticeQuestions(input: {
  ...
  const conditions: Prisma.Sql[] = [
    ...
    Prisma.sql`"classLevel" = ${input.classLevel}`,
    Prisma.sql`"subjectSlug" = ${input.subjectSlug}`,
  ]
  if (input.chapterSlug) conditions.push(Prisma.sql`"chapterSlug" = ${input.chapterSlug}`)
  if (input.examType) conditions.push(Prisma.sql`"examType" = ${input.examType}`)
  if (input.sourceType) conditions.push(Prisma.sql`"sourceType" = ${input.sourceType}`)

  try {
    return await db.$queryRaw<PracticeQuestion[]>(Prisma.sql`
...
export async function recordPracticeAnswer(input: {
  ...
  const rows = await db.$queryRaw<AnswerRow[]>`
    SELECT "id", "classLevel", "subjectSlug", "chapterSlug", "topicSlug",
           "correctAnswer", "solution", "explanation", "hint"
    FROM "AcademicQuestion"
  ...
  const timeTaken = Number.isFinite(input.timeTakenSeconds)
    ? Math.max(0, Math.round(input.timeTakenSeconds ?? 0))
    : null
  ...
  await db.$executeRaw`
    INSERT INTO "AcademicMasteryRecord" (
      "id", "userId", "classLevel", "subjectSlug", "chapterSlug", "topicSlug",
      "attempts", "correctAttempts", "accuracy", "masteryScore", "averageTimeSeconds",
      "lastPractisedAt", "updatedAt"
    ) VALUES (
      ${masteryId}, ${input.userId}, ${question.classLevel}, ${question.subjectSlug}, ${question.chapterSlug}, ${topicKey},
      1, ${isCorrect ? 1 : 0}, ${isCorrect ? 100 : 0}, ${isCorrect ? 20 : 5}, ${timeTaken}, NOW(), NOW()
    )
    ON CONFLICT ("userId", "classLevel", "subjectSlug", "chapterSlug", "topicSlug") DO UPDATE SET
