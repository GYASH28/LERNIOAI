        ...
        {mistakes.length ? (
          <div className="mt-7 space-y-3">
            {mistakes.map((mistake) => {
              const classLevel = profile.classLevel === 'DROPPER' ? '12' : profile.classLevel
              return (
                <article key={mistake.id} className="rounded-2xl border border-border bg-card p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-primary"><TriangleAlert className="h-4 w-4" /> {mistake.subjectSlug} · {mistake.chapterSlug.replaceAll('-', ' ')}</div>
                  ...
                  <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
                    ...
                    <Link href={`/practice/session?class=${classLevel}&subject=${mistake.subjectSlug}&chapter=${mistake.chapterSlug}`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary">Retry chapter <ArrowRight className="h-4 w-4" /></Link>
                  </div>
                </article>
              )
            })}
