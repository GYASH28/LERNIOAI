...
export async function POST(req: NextRequest) {
  try {
    ...
    let user = await db.user.findUnique({ where: { email: firebaseUser.email.toLowerCase() } })
    if (!user) {
    ...
    const response = NextResponse.json({ ok: true, data: { id: user.id, email: user.email, name: user.name, role: normalizeRole(user.role) } })
